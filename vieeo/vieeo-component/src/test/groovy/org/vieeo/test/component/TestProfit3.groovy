package org.vieeo.test.component;
...
class TestProfit3 {
	...
	@Test
	public void test() {
		BufferedReader reader = new File("D:\\新建文件夹\\profit_rule\\old.txt").newReader('gb2312');
		...
		BufferedReader reader2 = new File("D:\\新建文件夹\\profit_rule\\yc2.txt").newReader('utf-8');
		reader2.eachLine {line ->
			...
			String values495887YCTO = " VALUES(TO_CHAR (SYSDATE, 'yyyymmdd') || PPCPROFIT.SEQ_PROFIT_RULE_FEE_SPEC.NEXTVAL,'$cardYC',$kind,'$role',${ids.get(line)},'[$line]${names.get(line)}','$profitType',$profitValueYDTO,'$realRole','$merchant1','$merchant1$merchant1Name','$profitType',0,$validDate,null,null,$saleTo,SYSDATE,SYSDATE,1,'YD卡特殊分润政策-新增加外部中凯类商户-20131223')";
			...
			String values495887YCFROM = " VALUES(TO_CHAR (SYSDATE, 'yyyymmdd') || PPCPROFIT.SEQ_PROFIT_RULE_FEE_SPEC.NEXTVAL,'$cardYC',$kind,'$role',${ids.get(line)},'[$line]${names.get(line)}','$profitType',$profitValueYDFROM,'$realRole','$merchant1','$merchant1$merchant1Name','$profitType',0,$validDate,null,$saleFrom,null,SYSDATE,SYSDATE,1,'YD卡特殊分润政策-新增加外部中凯类商户-20131223')";
			...
			String values107697YCFROM = " VALUES(TO_CHAR (SYSDATE, 'yyyymmdd') || PPCPROFIT.SEQ_PROFIT_RULE_FEE_SPEC.NEXTVAL,'$cardYC',$kind,'$role',${ids.get(line)},'[$line]${names.get(line)}','$profitType',0,'$realRole','$merchant2','$merchant2$merchant2Name','$profitType',0,$validDate,null,null,null,SYSDATE,SYSDATE,1,'YD卡特殊分润政策-新增加外部中凯类商户-20131223')";
		...
		BufferedReader reader4= new File("D:\\yc2.2.txt").newReader('utf-8');
		reader4.eachLine {line ->
			String pre495887YCTO = "INSERT INTO PPCPROFIT.PROFIT_RULE_FEE_SPEC (ID,CARD_TYPE,INDUSTRY_ID,PROFIT_ROLE, PROFIT_USER_ID,PROFIT_USER_NAME,PROFIT_TYPE,PROFIT_VALUE,REL_PROFIT_ROLE,REL_PROFIT_USER_ID,REL_PROFIT_USER_NAME,REL_PROFIT_TYPE,REL_PROFIT_VALUE,VALIDATE_DATE,INVALIDATE_DATE,SALE_DATE_FROM,SALE_DATE_TO,CREATE_TIME,LAST_UPDATE_TIME,VERSION,RESV_FLD1)";
			String values495887YCTO = " VALUES(TO_CHAR (SYSDATE, 'yyyymmdd') || PPCPROFIT.SEQ_PROFIT_RULE_FEE_SPEC.NEXTVAL,'$cardYC',$kind,'$role',${ids.get(line)},'[$line]${names.get(line)}','$profitType',$profitValueYDTO,'$realRole','$merchant1','$merchant1$merchant1Name','$profitType',0,$validDate,null,null,$saleTo,SYSDATE,SYSDATE,1,'YD卡特殊分润政策-新增加外部中凯类商户-20131223')";
			println pre495887YCTO+values495887YCTO+";";
